...
function addExpenseToChart(name, amount) {
    ...
    let colorCount = expensesChart.data.datasets[0].backgroundColor.length// counts how many colors we have used so far

    // What's the next color?
    // cycle back to the start of the list, if they've all been used
    let color = chartColors[colorCount % chartColors.length] % // it is like division but gives the remainder back
...
addExpenseButton.addEventListener('click', function() {
    ...
    // Update chart
    addExpenseToChart(expenseName, expenseAmount)// calling the function
